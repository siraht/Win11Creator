Describe 'Offline servicing transaction boundary' {
    ...
    It 'rejects a synthetic Defender package instead of claiming client antivirus-core removal' {
        ...
        { Invoke-WinUtilOfflineServicingTransaction -InstallImagePath $script:wimPath -ImageIndex 6 -ResolvedPlan $plan -MountPath $script:mountPath -ManifestDirectory $script:manifestPath -SecurityOperation $operation } |
            Should -Throw '*no validated Microsoft-supported offline Defender antivirus core target*'
        Should -Invoke Mount-WindowsImage -Times 0 -Exactly
    }

    It 'rejects omitted, retargeted, and synthetic Defender operations before mounting' {
    ...
    It 'keeps synthetic Defender partial-removal input blocked before mounting' {
