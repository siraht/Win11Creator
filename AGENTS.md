...
- Keep the human Lean DAW ISO path in one persistent double-clickable window: the launcher owns elevation, file/edition/profile/output/driver selection, packaging-tool discovery, stage progress, live logs, result links, and temporary paths. Do not replace it with sequential dialogs, estimated-time screens, or a parameter-heavy command; reserve the latter for automation and tests.
...
- Keep every tab's required actions in a docked/table layout with its primary action inside the tab. Runtime bounds checks must follow an invisible real `Show()` and Windows message-loop pass; `CreateControl()`/`PerformLayout()` before first show does not establish reliable client geometry across DPI and window sizes.
