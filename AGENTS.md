...
- Keep the human Lean DAW ISO path inside WinUtil's existing compiled WPF `Win11ISO` workspace. A double-clickable command may bootstrap and navigate to that workspace, but it must not own a second UI, configuration model, progress system, or orchestration path; reserve headless tools for automation and tests.
- Preserve an Evaluation ISO's native edition/channel configuration. Pin the selected WIM index in `autounattend.xml`, remove stale `PID.txt`, and never synthesize a Retail `ei.cfg` for an `*Eval` EditionID.
- Keep small ISO repairs as a nested mode in the native WPF `Win11ISO` workspace. Compile repair functions from `functions/private/`, share actual media/packaging primitives with Build, and do not make WPF handlers depend on source-relative tool scripts.
- Extend upstream WinUtil surfaces before creating a replacement interface. Preserve its WPF controls, `$sync` state, dispatcher/runspace patterns, logging, progress, and ISO/USB lifecycle; retire duplicate UI code only after native parity is proven.
