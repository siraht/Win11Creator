...
**Current status:** Paused by the project owner on 2026-08-16 for continuation on a local Windows machine. Commits `3c34142` and `bf378f6` add and test `lean-daw-defender-retained`, an explicit Lean DAW variant whose only policy difference from `lean-daw` is `defender: keep`; the original Defender-removal profile remains unchanged and fail-closed. A live build of the retained-Defender variant reached exact setup staging, component cleanup, `CheckHealth`, mutation postconditions, and its single WIM commit. The host then exhausted validation space and QEMU paused on an I/O error; the same VM was resumed after reclaiming space, but the owner intentionally stopped the build before completion. No ISO or evidence set was published. At the owner's direction, the complete VPS validation environment was removed: the QEMU disk, source/base and built ISO files, answer media, source mount, software-TPM state, validation scripts/evidence, and temporary services. No validation mount, loop device, QEMU/TPM process, or retained ISO artifact remains. Resume on Windows from the committed repository and supply a fresh authenticated Windows 11 x64 25H2 build-26200 source ISO.
...
* `2026-08-24` — Added a double-click `Build-LeanDAW.cmd` path for the Defender-retained profile. The guided launcher requests elevation, opens source/output and edition selectors, discovers or offers to install `oscdimg`, owns a unique work directory, and removes that work only after ISO/evidence publication succeeds. Focused launcher/build/policy verification passed 38 tests; live Windows dialogs and a complete local build remain the next proof.
