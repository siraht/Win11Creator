...
* Treat Defender as a privileged operation that cannot fall through generic feature/package/service servicing. Source-shaped 25H2 evidence now shows no supported removable Windows 11 client AV-core target, so the catalog has no speculative Defender match and even a handcrafted `RemoveDefenderOffline` operation is rejected before mounting. OneDrive still uses only the built-in `System32\\OneDriveSetup.exe /uninstall` command at `specialize` in addition to its resolved offline targets.
...
* Require an exact before row and terminal after-state for every supported mutation, and require protected/kept/manual rows to remain unchanged. Any future supported privileged consumer must supply an equally strict after-state; currently Defender core removal is rejected before mutation. A command exit code without the requested state change is failure.
...
* Expand each basic `Get-WindowsImage` list record through an exact index-detail query before applying the whole-image support gate. Live Windows returns architecture, edition, and version only on the detailed object; absence on a basic record is not evidence that the image is unsupported.
* Treat StockControl as a source-integrity control, not a demand that every optional inbox consumer package exist in every Microsoft edition. A present stock target must be healthy/enabled, but a source-optional absent target is non-required; Lean removal/disable assertions remain required and unchanged.
* Do not reinterpret the removable `Microsoft.Windows.Sense.Client` capability or Sense FoD packages as the Defender antivirus platform. The official 25H2 image still contains `MsMpEng.exe`, `WdFilter.sys`, `WdNisDrv.sys`, and Security Health after the default-definitions feature disable attempt. Keep Lean blocked unless a supported consumer can prove the requested Defender after-state without manual WinSxS, driver, ACL, or servicing-database deletion.
* Match the live provisioned CoreAI AppX (`MicrosoftWindows.Client.CoreAI`) exactly in addition to the CBS/AIX package rules. Similarly named third-party packages remain unknown/manual, and a no-op CoreAI removal must discard the transaction.
* Require installed `DISM /English` health probes to emit the exact healthy state `No component store corruption detected.`. A zero exit code with repairable, corrupt, empty, or ambiguous output is not acceptance evidence.
...
* Lay out tab actions with a docked table rather than fixed right-edge coordinates, and keep the mode's primary action inside its tab. Before showing the form on Windows, switch through every tab and fail startup if any required action is missing, attached to the wrong tab, zero-sized, outside the configuration client area, or mislabeled.
...
* The earlier dedicated Defender consumer was a useful fail-closed experiment, but official source inventory invalidated its assumed target surface. `Windows-Defender-Default-Definitions` is definitions payload, while `Microsoft.Windows.Sense.Client~~~~` and Sense FoD packages are MDE/Sense; none removes AV core. Microsoft documents Defender feature uninstall for Windows Server, not Windows 11 client. The production contract therefore rejects this operation rather than mutating adjacent payloads or reaching for unsupported file/WinSxS/driver deletion.
...
* Windows PowerShell 5.1 compiled and parsed the generated script in the installed stock VM with zero parser errors. The stock Quick acceptance run then passed 46 checks with zero required failures; its 12 failures are intentionally non-required developer/DAW tools and its 15 `NotRun` results are source-optional or Release-depth probes.
* Live `Get-WindowsImage` behavior differs from the Linux fixtures that originally shaped the builder: the list call returns `BasicImageInfoObject` without architecture/version, while `-Index 1` returns `WimImageInfoObject` with architecture `9` and version `10.0.26200.6584`. The production metadata boundary now performs and validates that exact expansion.
* The official-source Lean transaction reached policy resolution, staged four exact setup actions, ran component cleanup and `CheckHealth`, and then failed its Defender postcondition because `Windows-Defender-Default-Definitions` was not `DisabledWithPayloadRemoved`. The transaction discarded the mount and published no output. Fresh source diagnostics found only the installed `Microsoft.Windows.Sense.Client~~~~` capability and Sense FoD packages as removable inventory, while Defender engine, filter/NIS drivers, and Security Health files remain present. This is the concrete restart blocker, not a missing orchestration layer.
* A resumed Default WinUtil control build proved the real transaction can analyze, clean, health-check, commit, and dismount the copied official WIM. Packaging then raised a native-command error at the `oscdimg.exe` boundary in `Invoke-WinUtilWindowsBuild.ps1`; cleanup removed the work directory, output, and pending evidence. This is the first repo-solvable task on resume, but it does not change the separate Defender servicing constraint.
* The packaging failure was not an `oscdimg.exe` failure: the ADK tool writes ordinary progress to stderr, and Windows PowerShell promoted that stream to `NativeCommandError` under `ErrorActionPreference=Stop` before the caller could inspect `$LASTEXITCODE`. Commits `fd4489d` and `72b5c74` preserve real stderr and exit semantics while removing blank `RemoteException` logging artifacts.
* Hashing a multi-gigabyte ISO back through QEMU's emulated SMB path is functionally correct but too slow for practical local proving. The rerun keeps the production publisher unchanged and performs output creation, hashing, and atomic evidence publication on the guest's local filesystem; only the already-verified artifact and evidence set are transferred afterward.
* WER, NFS, Windows Update installation, and developer toolchain acceptance now have strict functional Release boundaries with planted failure coverage. Until those boundaries run in the generated Lean VM, they prove harness behavior rather than the requested installed outcome.
* Profile import/export and comparison are real product features rather than report artifacts: imports are schema-validated complete catalog states, and comparison rows are generated from the same canonical profile resolver. Linux headless tests do not establish live Windows keyboard, screen-reader, dialog, or layout behavior.
...
* `2026-08-15` — Downloaded the official Microsoft Windows 11 Enterprise Evaluation 25H2 EN-US x64 ISO, visually checked Microsoft’s hash PDF, matched the published SHA-256 exactly, and confirmed its single WIM image reports x64 build `26200.6584`.
* `2026-08-15` — Installed that untouched image in a UEFI Secure Boot/TPM 2.0 KVM control VM. Windows 11 Enterprise Evaluation 25H2 build `26200.6584`, Secure Boot, TPM readiness, WinRE, Explorer, Start, Settings, update service availability, and Windows PowerShell 5.1 compile/parse all passed live probes.
* `2026-08-15` — Integrated live-harness corrections `293eaf2`/`7c966d5`: encoded Windows Update COM invocation, current WebView2 discovery, exact OneSettings/FlightSettings evidence, System32 OneDrive detection, and source-optional Stock semantics. The rerun produced harness `1.3.0`, 46 passes, 0 required failures, and durable JSON/log evidence. The focused installed/Hyper-V/sanity regression remained 43 passed, 0 failed, 2 unavailable Windows-PowerShell skips on Linux.
* `2026-08-15` — The first noninteractive live build exposed the basic-versus-detailed Windows image metadata split. Commit `ceaef80` now expands and validates every exact index before the unchanged x64/build-26200 gate; its focused builder suite passed 16 tests.
* `2026-08-16` — The next official-source Lean build resolved the real mounted inventory, staged four exact setup actions, completed cleanup and `CheckHealth`, then rejected `Windows-Defender-Default-Definitions` because the feature did not reach `DisabledWithPayloadRemoved`. The mount was discarded and no ISO/evidence directory was published. A fresh read-only diagnostic cycle captured the Sense capability/package and Defender file evidence, then discarded cleanly; `Get-WindowsImage -Mounted` returned no mounts at the pause boundary.
* `2026-08-16` — Integrated installed-health commits `804e1b3`/`04d6984`: acceptance harness `1.4.0` now requires explicit healthy CheckHealth/ScanHealth output. Its installed/Hyper-V/sanity regression discovered 48 tests: 46 passed, 0 failed, 2 unavailable Windows-PowerShell skips; compile and focused production analysis passed.
* `2026-08-16` — Integrated CoreAI commits `0b99139`/`98bef46`: the live `MicrosoftWindows.Client.CoreAI` provisioned AppX resolves to Lean removal, protected WebView2/Client.CBS remain untouched, and a planted no-op removal discards. The focused component/transaction run passed 43 tests; compile and catalog validation passed.
* `2026-08-16` — Integrated Defender source-investigation commits `dd80878`/`3cc0e84`: removed false AV-core wildcards, planted official-source-shaped negatives, and rejected unsupported synthetic Defender core operations before mount. The focused policy/live/transaction suite passed 84 tests with 0 failures; compile passed and focused production analysis reported no new actionable findings.
* `2026-08-16` — Ran a Default WinUtil build from the authenticated official source. The transaction completed cleanup and CheckHealth, committed once, and left zero mounted images. `oscdimg.exe` then surfaced a native-command error at `tools/Invoke-WinUtilWindowsBuild.ps1:83`; failure cleanup removed the work directory and published neither an ISO nor evidence. The VM and temporary validation services were stopped, and the owner paused the project at this recoverable boundary.
* `2026-08-16` — Integrated `fd4489d` and `72b5c74`: native `oscdimg.exe` stderr no longer creates false failure under Windows PowerShell, exact nonzero exits remain blocking, and blank native records no longer serialize as `RemoteException`. The focused packaging suite passed 19 tests and compile/production analysis passed.
* `2026-08-16` — Integrated functional installed-acceptance slices for WER (`d239255`/`838b198`), NFS (`7761fcb`/`9cc7a0d`/`d020f08`), Windows Update installation (`a24449b`/`45ddc47`/`7c2d806`/`60cad1c`), and developer tools (`9372084`/`53c3761`/`6a00140`). Harness `1.8.0` keeps Quick passive and makes malformed, failed, timed-out, or unclean Release probes blocking; the integrated installed/Hyper-V suite passed 71 tests with no failures.
* `2026-08-16` — Integrated Custom/profile usability work: `b361bbd` makes manual policy edits enter a validated Custom state; `5293cd3`/`cee2181`/`f36835e` add strict versioned preset import/export; and `98d3819`/`c02ad15`/`52e8c2f` add canonical visual profile comparison. The final focused compile plus policy/preset/comparison/XAML run passed 62 tests, with only two unavailable Windows-PowerShell parser skips.
* `2026-08-16` — The first resumed Default build crossed the former native packaging failure and completed `oscdimg.exe`, proving the root-cause fix on real ADK tooling. Its evidence publisher then encountered the emulated SMB read bottleneck, so the unverified output was retired without evidence publication. A fresh official-source Default rerun began from archive SHA-256 `CFE6016E1B4D3807E2170F67C662298AAEF000B62D7175149A2FC95BF33D4981`, using the unchanged production publisher against guest-local output before transfer.
* `2026-08-16` — At the owner's requested stop boundary, the local-output rerun had completed cleanup, CheckHealth, its single WIM commit, and dismount. Packaging was stopped only after Windows reported zero mounted images and a detached source ISO; unpublished guest work/output were cleaned, the VM/software TPM and temporary file services were stopped, and no ISO/evidence claim was made. The source-version-comparison worker was interrupted before integration. Resume from the pause-checkpoint commit containing this entry.
* `2026-08-16` — Added `lean-daw-defender-retained` in `3c34142`/`bf378f6`. It preserves every Lean DAW action except Defender, which is explicitly kept; 92 focused tests passed, 0 failed, and 2 Windows-PowerShell parser checks were unavailable on Linux. A live build reached servicing commit but was intentionally aborted at the owner's request before ISO/evidence publication. The owner then directed removal of all VPS validation media and VM state; the QEMU disk, source/base and output ISOs, answer media, mounts, TPM state, scripts/evidence, and services were removed. Continue on a local Windows machine with a newly supplied authenticated source ISO.
* `2026-08-24` — Replaced the sequential launcher dialogs/console flow with one persistent `Build-LeanDAW.cmd` window. It requests elevation; analyzes the chosen ISO; exposes edition, canonical profile, output, and optional driver choices; explains and blocks unsupported Defender removal; discovers or offers to install `oscdimg`; tracks durable stages and the live build log asynchronously; prevents closing mid-build; and links the finished ISO/evidence. Successful builds alone request cleanup of their unique work directory. Focused launcher/build/policy verification passed 41 tests, compile passed, and both changed production scripts passed focused analysis; live Windows rendering and a complete local build remain the next proof.
* `2026-08-24` — Fixed Enterprise Evaluation installation from generated media: the builder had pinned the correct WIM index and removed `PID.txt`, but then replaced evaluation media metadata with a synthesized Retail `ei.cfg`, producing Setup's “failed to validate the product key” error. Evaluation editions now preserve the source configuration while retaining exact index pinning and a key-free answer file. The focused setup/build regression passed 48 tests and compile passed; the regenerated ISO still requires a live Ventoy install retry.
* `2026-08-25` — Added **Repair existing ISO** to the same `Build-LeanDAW.cmd` window; no second command or user flow was created. The data-driven repair engine currently implements only the observed Enterprise Evaluation product-key fix, validates applicability before copying, never services `install.wim`, preserves the original ISO, creates a new dual BIOS/UEFI image, and cleans partial output on failure. The integrated launcher/build/repair/setup run passed 69 tests; compile and focused production analysis passed. A real Windows repack and Ventoy install remain required.
* `2026-08-25` — Fixed the first live Repair inspection failure: the repair provider had been dot-sourced into a startup function and was therefore invisible to the later Windows Forms click handler. Both internal engines now load at launcher script scope, and a planted command-resolution assertion covers the event-handler contract. The integrated run passed 70 tests; compile and focused analysis passed.
...
* [x] **Preset import/export.**
  ...
  * **Proof:** `5293cd3` adds the versioned schema/serializer, `cee2181` wires WPF file controls, and `f36835e` enforces complete exact catalog actions and exclusive groups. The preset suite passed all 8 tests, including round-trip and malformed/unknown-field negatives.

* [x] **Diff two profiles visually.**

  * **Proof required:** UI test.
  * **Proof:** `98d3819` builds deterministic canonical comparison rows, `c02ad15` adds accessible WPF selectors/results, and `52e8c2f` exercises the UI state headlessly. The integrated profile-diff model/UI tests passed all 7 cases; live WPF layout remains part of Windows UI proving.
