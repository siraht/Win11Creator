...
**Current status:** Paused by the project owner at a clean servicing boundary on 2026-08-16. The policy-driven Analyze-to-Build path, typed offline/setup/security actions, two-mode UAC choice, x64 25H2 source gate, source-format preparation, atomic one-mount transaction, noninteractive Stock/Lean ISO builder, artifact publication, and installed-state/clean-VM harness are integrated. A real stock 25H2 control VM passes all Quick-mode required checks. The first official-source Lean build was correctly discarded because Windows did not reach the required Defender payload-removed after-state; no Lean ISO or success evidence was published, and no WIM remains mounted. Resume at the Defender servicing constraint described below, then repeat the Lean build/install/acceptance gates.
