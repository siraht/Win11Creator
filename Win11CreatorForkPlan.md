...
**Current status:** Active on 2026-08-16 at integration revision `52e8c2f`. The native `oscdimg.exe` boundary now captures valid stderr without converting it into a false terminating error and normalizes native output records before durable logging. A resumed official-source Default build crossed the former packaging failure and created an ISO, but publication over emulated SMB exposed an impractically slow read/hash path; that unverified output was retired without publishing evidence, and a full rerun is in progress with ISO creation, hashing, and evidence publication local to the Windows guest before transfer. The installed-acceptance harness now has fail-closed Release probes for WER crash-dump capture, reversible NFS use, one real Windows Update installation (or exact zero-applicable evidence), and functional Git/PowerShell/Node/Bun/Python/Rust toolchains. Custom profile state, strict preset import/export, and visual profile comparison are integrated and headlessly tested. A real untouched stock 25H2 control VM still passes every Quick-mode required check. The full Lean DAW scope is unchanged: official-source investigation found no Microsoft-supported Windows 11 client servicing identity for removing Defender antivirus core, so Lean remains blocked before mounting rather than claiming that Sense/default-definitions removal or passive mode satisfies Defender absence.
