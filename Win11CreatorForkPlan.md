...
**Current status:** Ready for the next local-Windows build attempt, but not yet release-proven. `lean-daw-defender-retained` preserves every Lean DAW action except Defender, which is explicitly kept; the original Defender-removal profile remains visible but fail-closed because the official 25H2 client image exposes no supported removable AV-core target. The root `Build-LeanDAW.cmd` now opens one persistent builder window for source ISO analysis, edition/profile/output/optional-driver selection, stage progress, live durable log output, and completed ISO/evidence links. Focused launcher/build/policy verification passes on portable PowerShell, while the Windows Forms interaction and a complete ISO/evidence publication still require the local Windows machine. No prior VPS VM, source ISO, output ISO, mount, or validation artifact remains.
